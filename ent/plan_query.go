...
func (_q *PlanQuery) QueryFormularyEntries() *FormularyEntryQuery {
...
func (_q *PlanQuery) WithFormularyEntries(opts ...func(*FormularyEntryQuery)) *PlanQuery {
	query := (&FormularyEntryClient{config: _q.config}).Query()
...
func (_q *PlanQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Plan, error) {
	...
	if query := _q.withFormularyEntries; query != nil {
		if err := _q.loadFormularyEntries(ctx, query, nodes,
			func(n *Plan) { n.Edges.FormularyEntries = []*FormularyEntry{} },
			func(n *Plan, e *FormularyEntry) { n.Edges.FormularyEntries = append(n.Edges.FormularyEntries, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PlanQuery) loadFormularyEntries(ctx context.Context, query *FormularyEntryQuery, nodes []*Plan, init func(*Plan), assign func(*Plan, *FormularyEntry)) error {
	...
	query.Where(predicate.FormularyEntry(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(plan.FormularyEntriesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.plan_formulary_entries
		if fk == nil {
			return fmt.Errorf(`foreign-key "plan_formulary_entries" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "plan_formulary_entries" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
